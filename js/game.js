...
const BASE_GROUND_Y = 410; // where ground rooms sit in your PC design
...
const MOBILE_LAYOUT = {
    books: { x: 300, y: 280 },
    borrow: { x: 900, y: 280 },
    security: { x: 1500, y: 280 },
    catalog: { x: 2200, y: 280 },
    staff: { x: 3000, y: 280 },
    archive: { x: 3600, y: 500 }
};
...
interactBtn?.addEventListener("click", () => {
    ...
    if (state.currentRoom) {
        ...
        const room =
            roomSounds[state.currentRoom.id]
...
function showDayEndScreen(text, callback) {
    ...
    }, 2000) // 👈 controls how long it stays
}
...
function getGroundY() {
    ...
    if (window.innerWidth <= 768) {
        return window.innerHeight * 0.68; // mobile
    }

    return window.innerHeight * 0.60; // desktop
}
...
function checkRooms() {
    ...
    const playerWorldX =
        (window.innerWidth / 2) - state.worldX;
...
const PLAYER_POSITION = {
    ...
    desktop: {
        x: 50,   // percentage
        y: 120    // pixels from bottom
    },

    mobile: {
        x: 40,   // move slightly left on mobile
        y: 210   // higher from bottom
    }
...
function getPlayerPosition() {
    ...
    return window.innerWidth <= 768
        ? PLAYER_POSITION.mobile
        : PLAYER_POSITION.desktop
...
const load = setInterval(() => {
    ...
    if (progress >= 100) {
        ...
        showDialogue(
            "Day 1: Your shift as librarian begins..."
        )

        showDialogue(
            "Investigate the library and gather 4 clues."
        )
    }
}, 50)
                ...
                action: () => {
                    ...
                    if (
                        state.clues.includes(
                            "Missing Book Slot Found"
                        )
                    ) {

                        showDialogue(
                            "You already checked this shelf."
                        )
                    ...
                    showDialogue(
                        "Something was removed recently."
                    )
                }
            },
            {
...
function checkStoryProgress() {
    ...
    if (
        ...
        showDialogue(
            "Several records now point to the same missing item..."
        )
